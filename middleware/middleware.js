
const config   = require("../config/config");
const jwt   = require('jsonwebtoken');
const responseCode   = require("../common/response_code");


exports.createJwtToken = async (user) =>
{
 
      return new Promise(async (resolve, reject) => {
        try {
          jwt.sign({ user }, config.JwtSupersecret, { expiresIn: config.JwtExpiration }, (err, token) => {
            resolve(token)
          });
        } catch (ex) {
          console.log("WriteOnJSONfile function error", ex)
        }
      })
    },

    
exports.createJwtToken = async (req, res, next) =>
{


      return new Promise(async (resolve, reject) => {

          if (typeof req.headers.authorization != "undefined" && req.headers.authorization != "") {
            try {
              jwt.verify(req.headers.authorization, config.JwtSupersecret, async (err, response) => {
                if (response) {
                  next(); //resolve(true);
                }
                else {
                  console.error(err)
                  responseCode.customResponse(res, 400);
                }
              })
            } catch (ex) {
              console.log("WriteOnJSONfile function error", ex)
              responseCode.customResponse(res, 400, ex);
            }
          } else {
            responseCode.customResponse(res, 401);
          }        
      })
    }
